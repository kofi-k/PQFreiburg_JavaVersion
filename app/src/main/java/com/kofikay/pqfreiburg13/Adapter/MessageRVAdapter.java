...
import com.kofikay.pqfreiburg13.MessageModal;
...
public class MessageRVAdapter extends RecyclerView.Adapter {
        ...
        switch (modal.getSender()) {
            case "user":
                ...
                if (currentUser.getPhotoUrl() != null) {
                    Picasso.get().load(currentUser.getPhotoUrl()).into(((UserViewHolder) holder).userChatPhoto);
                } else {
                    setImage(((UserViewHolder) holder).userChatPhoto);
                }
                Picasso.get().load(currentUser.getPhotoUrl()).into(((UserViewHolder) holder).userChatPhoto);
                break;
            case "bot":
    ...
    public static class UserViewHolder extends RecyclerView.ViewHolder {
        ...
        // for our text view.
        TextView userTV;
        CircleImageView userChatPhoto;

        public UserViewHolder(@NonNull View itemView) {
            super(itemView);
            // initializing with id.
            userTV = itemView.findViewById(R.id.idTVUser);
            userChatPhoto = itemView.findViewById(R.id.userChatPhoto);
        }
    }
    ...
    public void setImage(CircleImageView userImage) {
        Drawable color = new ColorDrawable(context.getResources().getColor(R.color.grey));
        Drawable image = userImage.getResources().getDrawable(R.drawable.user);
        LayerDrawable layerDrawable = new LayerDrawable(new Drawable[]{color, image});
        Glide.with(this.context).load(layerDrawable).into(userImage);
    }
